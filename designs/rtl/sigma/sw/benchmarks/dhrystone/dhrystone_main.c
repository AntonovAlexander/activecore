  ...
  /* Main and Proc_0 in the Ada version             */
{
 ...
 // debug_printf("Dhrystone Benchmark, Version %s\n", Version);
  ...
  while (!Done) {
    ...
    SFR_TIMER_PERIOD = 0xFFFFFFFF;
    SFR_TIMER_CTRL = 0x1;


    for (Run_Index = 1; Run_Index <= Number_Of_Runs; ++Run_Index)
    {
       *((volatile int*)0x80000000) = Run_Index;
...
*/
  ...
  io_buf_int[0] = Microseconds;
  io_buf_int[1] = Dhrystones_Per_Second;
  

  io_buf_int[2] = Int_Glob;
  io_buf_short[6] = Ch_1_Glob;
  io_buf_int[6] = HZ;

  //printf("Microseconds for one run through Dhrystone: %ld\n", Microseconds);
  //printf("Dhrystones per Second:                      %ld\n", Dhrystones_Per_Second);
  *((volatile int*)0x80000000) = 0xFFFFFFFF;
  return 0;
}
