...
open class MultiExu(val name : String, val Exu_cfg_rf : Exu_CFG_RF, val MultiExu_cfg_rf : MultiExu_CFG_RF, val rob_size : Int) {
    ...
    */
    ...
    fun translate_to_cyclix(DEBUG_FLAG : Boolean) : cyclix.Generic {
        ...
        rf_dim.add(31, 0)
        rf_dim.add(31, 0)
        var RF = cyclix_gen.uglobal("genrf", rf_dim, "0")
        ...
        rob_struct.addu("exec",     0, 0, "0")
        rob_struct.addu("rf_we",       0,  0, "0")
        rob_struct.addu("rf_addr",    input_rf_addr_width-1, 0, "0")
        rob_struct.addu("rf_wdata",    MultiExu_cfg_rf.input_RF_width-1, 0, "0")
        ...
        var exu_req = cyclix_gen.global(cyclix_gen.GetGenName("exu_req"), Exu_cfg_rf.req_struct)
        var exu_resp = cyclix_gen.global(cyclix_gen.GetGenName("exu_resp"), Exu_cfg_rf.resp_struct)
        ...
        run {
            cyclix_gen.begif(cyclix_gen.subStruct(rob_head, "exec"))
            run {
                // wb to RF
                cyclix_gen.assign(
                    RF,
                    hw_fracs(hw_frac_V(cyclix_gen.subStruct(rob_head, "rf_addr"))),
                    cyclix_gen.subStruct(rob_head, "rf_wdata"))
            }; cyclix_gen.endif()
            cyclix_gen.begelse()
            run {
                cyclix_gen.begif(cyclix_gen.subStruct(rob_head, "rf_we"))       // "load"
                run {
                    cyclix_gen.assign(
                        RF,
                        hw_fracs(hw_frac_V(cyclix_gen.subStruct(rob_head, "rf_addr"))),
                ...
                cyclix_gen.begelse()                                                        // store
                run {
                    cyclix_gen.assign(
                        rob_head,
                        hw_fracs(hw_frac_SubStruct("rd_wdata")),
                        cyclix_gen.indexed(RF, cyclix_gen.subStruct(rob_head, "rf_addr")))
                }; cyclix_gen.endif()
            }; cyclix_gen.endif()

            cyclix_gen.begif(cyclix_gen.fifo_wr_unblk(cmd_resp, cyclix_gen.subStruct(rob_head, "rf_wdata")))
            run {
                // shifting ops
        ...
        run {
            ...
            cyclix_gen.assign(
                rob,
                hw_fracs(hw_frac_V(rob_wr_ptr), hw_frac_SubStruct("rf_wdata")),
